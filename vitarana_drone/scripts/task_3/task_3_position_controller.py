...
def main():
    ...
    for i in range(len(e_drone.buildings.keys())):
        ...
        rospy.loginfo("Bupesh, going to Building "+ str(e_drone.current_marker_id)+ " at "+ str(e_drone.setpoint_final))
        reach_destination(flag = 1, speed = 5)
        # To settle on the destination
        stablize_drone(time_limit = 5)
        rospy.loginfo("Reached Building BOSS "+ str(e_drone.current_marker_id))
